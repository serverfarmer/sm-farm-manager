...
if [ "$3" = "" ]; then
	...
	exit 1
elif [ "`getent hosts $3`" = "" ]; then
	echo "error: host $3 not found"
	exit 1
fi
...
ssh -i $rkey -p $rport root@$rhost "useradd -u $uid -d $path -M -g imapusers -s /bin/false imap-$1"
ssh -i $rkey -p $rport root@$rhost "usermod -G www-data -a imap-$1"
